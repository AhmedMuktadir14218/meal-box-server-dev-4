...
const updateMeal = async (req: Request, res: Response) => {
    try {
      ...
      // Check if the logged-in provider is the creator of the meal
      if (meal.providerId.toString() !== req.user.id) {
        return res.status(403).json({ success: false, message: 'Unauthorized' });
      }
  
      const updatedMeal = await MealMenuService.updateMeal(req.params.mealId, req.body);
  ...
  const deleteMeal = async (req: Request, res: Response) => {
    try {
      const meal = await <MealMenuService className="getMealById"></MealMenuService>(req.params.mealId);
      if (!meal) {
        return res.status(404).json({ success: false, message: 'Meal not found' });
      }
